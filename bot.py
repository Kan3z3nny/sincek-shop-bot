...
def handle_ss(message):
    ...
    if uid in user_orders:
        ...
        # Admin text with Item and ID included in callback data
        ...
        mk = types.InlineKeyboardMarkup()
        # Data format: adm_action_uid_item_gameid
        mk.add(types.InlineKeyboardButton("⌛ Approve (စစ်နေဆဲ)", callback_data=f"adm_chk_{uid}"),
               types.InlineKeyboardButton("✅ Success (အောင်မြင်)", callback_data=f"adm_done_{uid}"))
        mk.add(types.InlineKeyboardButton("❌ Reject (ပယ်ချ)", callback_data=f"adm_rej_{uid}"))
...
def admin_action(call):
    ...
    # Extract item and ID from admin caption for the Success message
    caption = call.message.caption
    # Simple extraction logic from the caption text
    try:
        item = caption.split("Item: ")[1].split("\n")[0]
        game_id = caption.split("GameID: ")[1].split("\n")[0]
    except:
        item = "Unknown"
        game_id = "Unknown"
    ...
    try:
        if action == "chk":
            ...
            bot.answer_callback_query(call.id, "Sent: Checking Message")
            
        elif action == "done":
            success_msg = (f"✅ <b>ဝယ်ယူမှု အောင်မြင်ပါသည်</b>\n\n"
                           f"🎮 Game: <b>MLBB</b>\n"
                           f"📦 Product: <b>{item}</b>\n"
                           f"🆔 ID: <code>{game_id}</code>\n\n"
                           f"ကျေးဇူးတင်ပါသည်ခင်ဗျာ။ 🙏")
            bot.send_message(target_uid, success_msg, parse_mode="HTML")
            bot.answer_callback_query(call.id, "Sent: Success Message")
            ...
            bot.answer_callback_query(call.id, "Sent: Reject Message")
            bot.edit_message_reply_markup(ADMIN_ID, call.message.message_id, reply_markup=None)
